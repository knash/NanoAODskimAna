...
if jobarr[0]>jobarr[1]:
	...
	sys.exit()
setit=(options.set).split(",")
if options.set=="ALLSIGHT":
	NanoF = NanoAODskim_Functions(options.anatype,options.era)
	setit=NanoF.allsignamesHT

if options.set=="ALLSIGZT":
	NanoF = NanoAODskim_Functions(options.anatype,options.era)
	setit=NanoF.allsignamesZT
for curset in setit:
	setname = curset
	...
	pdsel = 	'''
			...
			float pdfweight;
			for(uint ilw=1;ilw<101;ilw++)	{
				pdfweight+=(LHEPdfWeight[ilw]-LHEPdfWeight[0])*(LHEPdfWeight[ilw]-LHEPdfWeight[0]);
							}
			pdfweight=sqrt(pdfweight)/std::fabs(LHEPdfWeight[0]);
			return std::min(pdfweight,float(3.0));
			'''
