...
if True:
	...
	if options.submit:
		...
		with open('temp.listOfJobs', 'w+') as tempcommands:
		    ...
		    if subsignal:
			    ...
			    if options.anatype=="tHb":
					...
					for thsig in thsigs:
						tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  '+thsig+'  --'+macro+' -e '+options.era+' -S '+searchstring+' -f /eos/cms/store/group/phys_b2g/knash --condor\n')
					
			    
			    if options.anatype=="tZb":
					tzsigs = NanoF.allsignamesZT 
					for tzsig in tzsigs:
						tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  '+tzsig+'  --'+macro+' -e '+options.era+' -S '+searchstring+' -f /eos/cms/store/group/phys_b2g/knash --condor\n')
		    ...
		    if subqcd:
			    if options.era=="2016":
				    ...
				    for j in xrange(numfqcd[0]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1000to1500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[0])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[1]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1500to2000_TuneCUETP8M1_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[1])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[2]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT2000toInf_TuneCUETP8M1_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[2])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')		
			    if options.era=="2017":
				    print "\t...QCD"
				    for j in xrange(numfqcd[0]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1000to1500_TuneCP5_13TeV-madgraph-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[0])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[1]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1500to2000_TuneCP5_13TeV-madgraph-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[1])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[2]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT2000toInf_TuneCP5_13TeV-madgraph-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[2])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')		
			    if options.era=="2018":
				    print "\t...QCD"
				    for j in xrange(numfqcd[0]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1000to1500_TuneCP5_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[0])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[1]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT1500to2000_TuneCP5_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[1])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
				    for j in xrange(numfqcd[2]):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  QCD_HT2000toInf_TuneCP5_13TeV-madgraphMLM-pythia8 -j '+str(j+1)+' -t '+str(numfqcd[2])+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+'  --condor\n')
		    if subttbar:
			    if options.era=="2016":
				    print "\t...ttbar"
				    for j in xrange(numfttbar1000):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  TT_Mtt-1000toInf_TuneCUETP8M2T4_13TeV-powheg-pythia8 -j '+str(j+1)+' -t '+str(numfttbar1000)+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+' '+' --condor\n')
				    for j in xrange(numfttbar700):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  TT_Mtt-700to1000_TuneCUETP8M2T4_13TeV-powheg-pythia8 -j '+str(j+1)+' -t '+str(numfttbar700)+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+' '+' --condor\n')

			    else:
				    print "\t...ttbar"
				    for j in xrange(numfttbarH):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  TTToHadronic_TuneCP5_13TeV-powheg-pythia8 -j '+str(j+1)+' -t '+str(numfttbarH)+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+' '+' --condor\n')
				    for j in xrange(numfttbarS):
				    	tempcommands.write('python ./tardir/'+pyfile+' -a '+options.anatype+' -s  TTToSemiLeptonic_TuneCP5_13TeV-powheg-pythia8 -j '+str(j+1)+' -t '+str(numfttbarS)+'  --'+macro+' -e '+options.era+' -f /eos/cms/store/group/phys_b2g/knash -S '+minset+' '+' --condor\n')
		    if subgjets:  
			    print "\t...gjets" 
		...
		commands.append("tar czvf tarball.tgz "+pyfile+" PhysicsTools NanoAODskim_RateMaker__*.root NanoAODskim_PUMaker__*.root NanoAODskim_TrigMaker__*.root NanoAODskim_Functions.py NanoAODskim_Data.py") 
				...
				if int(cjob)!=len(fileset):
					...
					for jj in xrange(int(cjob)):
						if not ((sets[-1] + "job"+str(jj+1)+"of"+cjob+".root") in fileset):
							print "output_$(JID)        python ./tardir/"+pyfile+" -a "+options.anatype+" -s  "+sets[-1]+" -j "+str(jj+1)+" -t "+cjob+"  --"+macro+" -e "+options.era+" -f /eos/cms/store/group/phys_b2g/knash -S "+minset+"  --condor"
