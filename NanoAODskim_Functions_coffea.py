...
class NanoAODskim_Functions_Coffea:
	def __init__(self,anatype="Pho",era="2017",versionstring="v8",settype="ttbar"):
		self.isdata=False
		if settype=="JetHT":
			self.isdata=True
		...
		if self.era=="2016":
			...
			self.etrigs = ["HLT_DiPFJetAve500","HLT_AK8PFHT800_TrimMass50","HLT_AK8PFHT750_TrimMass50","HLT_AK8PFJet360_TrimMass30","HLT_AK8PFJet400_TrimMass30"]
		...
		#self.jmeCorrections = createJMECorrector((not self.isdata), "2017", "B", "Total", True, "AK8PFPuppi", False)
		if anatype=="Pho":
			self.labels = ["W","P","PW"]
			self.ak8labels = ["W","P"]
			self.ak4labels = []
			...
			cutranges =	{
					...
					'msoftdrop__H':{'L':[10.0,30.0],'M':[105.0,150.0],'M1':[105.0,150.0],'T':[105.0,150.0],'F':[130.0,250.0]},
					...
					'btagDeepFlavB__B':{'L':[-1.0,0.0494],'M':[-1.0,0.0494],'M1':[0.0494,1.0],'T':[0.0494,1.0],'F':[-1.0,0.0494]},
	...
	def ttreeinit(self,curttree,runver=""):
		...
		intlist = ["pdgId","cutBasedV1Bitmap","statusFlags"]
	...
	def physobjinit(self,lvdict,num=10):
		...
		for cname in lvdict:
			...
			#print( cname
			otherb.append(cname)
		for lv in range(min(len(lvdict["pt"]),num)):
			#print( lv
	...
	def tptrw(self,geninfo):
		...
		for gg in geninfo:
			#print( gg["pdgId"],gg["statusFlags"],gg
