...
for curset in (options.set).split(","):
	...
	#init jet correctors
	recal = NanoF.jmeCorrections().jetReCalibrator
	recalak4 = NanoF.jmeCorrectionsak4().jetReCalibrator
	...
	for curfilename in rdffiles:
		...
		if NanoF.isdata:
			...
			for pars in parsename:
				if pars.find("Run201")!=-1:
					runlet=pars[11:]
					break
			NanoF.jmeCorrections = createJMECorrector(False, NanoF.era, runlet, "Total", True, "AK8PFPuppi", False)
			NanoF.jmeCorrectionsak4 = createJMECorrector(False, NanoF.era, runlet, "Total", True, "AK4PFchs", True)
		...
		for iev in xrange(nent):
			...
			for shift in shiftuncert:
				...
				jscalefac=1.0
				jsmearfac=1.0
				if ((shtype=="jes") and (not NanoF.isdata)) or (NanoF.era=="2018"):
					...
					for subj in ak8subjets:
						(jet_pt, jet_mass) = recalak4.correct(subj,rho,delta=delta)
				...
				if not NanoF.isdata:
					...
					NanoF.JerSmearstoc(ak8jets,rho,jind,jtype="ak8")
					NanoF.JerSmearstoc(ak4jets,rho,jind,jtype="ak4")
				...
				for iak8,ak8j in enumerate(ak8jets):
					jets_msoftdropdef_nom=NanoF.CorrectSdMassdef(ak8jets,ak8subjets)
        				...
        				#print ak8j.msoftdropdef,jets_msoftdrop_raw[iak8],ak8j.msoftdrop
				htval = 0
