...
def build_allhad_model(TT_SF_local=1.0,ltype="Mu"):
    files = ["ThetaFile_ttfit_Mu2017_Pt700to1400_CNN0p9.root"]
    ...
    for ob in model.observables:
		...
		if ptype=="bkg":
    			model.add_lognormal_uncertainty(ptype+'_rate', math.log(2.0), pr,ob)
...
posttext = "ThetaFile_ttfit_Mu2017_Pt700to1400_CNN0p9.root".replace("ThetaFile_ttfit_PSET_default","").replace(".root","").replace("Mu","").replace("Ele","")
