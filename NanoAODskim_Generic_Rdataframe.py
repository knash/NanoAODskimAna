...
settype=NanoF.setfilter(setname)
...
#print files
df= ROOT.ROOT.RDataFrame("Events",filenames)
presel =[	
		"nFatJet>=2",
		"HLT_PFHT1050 > 0",
		"CustomAK8Puppi_pt[1]>"+str(minpt),
		"(CustomAK8Puppi_eta[0]<2.4) && (CustomAK8Puppi_eta[1]<2.4)",
		"int npmu = 0 ; for(int nmu=0;nmu<nMuon;nmu++){ if (((Muon_pt[nmu]>70.0) && ((Muon_mvaId[nmu]>1) || (Muon_highPtId[nmu]>1))) ) npmu+=1;}return npmu==0;",
		"int npel = 0 ; for(int nel=0;nel<nElectron;nel++){ if (((Electron_pt[nel]>70.0) && ((Electron_mvaFall17V2Iso_WP90[nel]>0) || (Electron_mvaFall17V2noIso_WP90[nel]>0)))) npel+=1;}return npel==0;"
	]
filtarr = [df]
...
FatJetpthisto= filtarr[-1].Histo1D('FatJet_pt')
#FatJetpthisto.Draw()
...
output = ROOT.TFile("NanoAODskimRDATAFRAME_"+options.anatype+macro+options.era+"__"+setnametowrite+jobstr+".root","recreate")
output.cd()
FatJetpthisto.Write()
output.Write()
output.Close()
